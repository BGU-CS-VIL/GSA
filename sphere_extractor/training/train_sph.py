...
from datasets.SPair_custom import SPairCustomDataset
...
def train(cfg):
    ...
    assert cfg['data']['set'] in ['SPair', 'FCars',"SPair_custom"], f"""{cfg['data']['set']} is not a valid training set"""
    if cfg['data']['set'] == 'SPair':
        TrainSet = SPairDataset
    elif cfg['data']['set'] == 'FCars':
        TrainSet = FreiburgCarsDataset
    elif cfg['data']['set'] == 'SPair_custom':
        TrainSet = SPairCustomDataset
    ...
    for e in (pbar_epoch := tqdm(range(epochs), dynamic_ncols=True)):
        ...
        if e % cfg['logs']['save_frequency'] == 0:
            model.make_checkpoint(exp_dir + f'/ckpts/{e}.pth')
