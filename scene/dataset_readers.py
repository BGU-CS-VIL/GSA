...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, semantic_feature_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # roy
        semantic_feature_path = os.path.join(semantic_feature_folder, image_name)
        # Check for both possible file patterns
        pca_path = semantic_feature_path + '_pca_fmap_CxHxW.pt'
        regular_path = semantic_feature_path + '_fmap_CxHxW.pt'
        sphere_path = semantic_feature_path + '_sphere_fmap_CxHxW.pt'
        # print("semantic_feature_path:", semantic_feature_path)
        # Use whichever file exists, preferring PCA if both exist
        if os.path.exists(pca_path):
            semantic_feature_path = pca_path
        elif os.path.exists(regular_path):
            semantic_feature_path = regular_path
        elif os.path.exists(sphere_path):
            semantic_feature_path = sphere_path
        else:
            raise FileNotFoundError(f"Neither PCA nor regular feature maps found for {image_name}")
        # print("final semantic_feature_path:", semantic_feature_path)
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    semantic_feature_dir = "spherical_feature_maps"
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            if not os.path.exists(cam_name):
                # if the path name by mistake is cs_storage/amoyalr/Research/Thesis/3gs_tools/feature-3dgs/datasets/objaverse/chair/0144133a874d4fa9881d8947928999e0/./train/frame_1.png.png'
                # lets remove the extra .png and fix the ./train/ to /train/
                cam_name = cam_name.replace(".png.png", ".png")
                cam_name = cam_name.replace("./train/", "train/")
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    if os.path.exists(os.path.join(path, "transforms_train.json")):
        ...
        train_cam_infos = readCamerasFromTransforms(path, "transforms.json", white_background, semantic_feature_folder=os.path.join(path, semantic_feature_dir))
